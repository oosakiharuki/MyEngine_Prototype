...
StructuredBuffer<Wall> gMatrixPalette : register(t0);
...
VertexShaderOutput main(VertexShaderInput input)
{
    ...
    output.position = mul(skinned.position, gTranfsformationMatrix.WVP);
    output.texcoord = input.texcoord;
    output.normal = normalize(mul(skinned.normal, (float32_t3x3) gTranfsformationMatrix.World));
    output.worldPostion = mul(skinned.position, gTranfsformationMatrix.World).xyz;
    return output;
}